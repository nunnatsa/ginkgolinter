...
// importing gomega/ginkgo and "wrappers" works, then invokes the
...
func run(t *testing.T, code string, cb func(file *ast.File, pass *analysis.Pass) error) {
	...
	wrappersDir := path.Join(tmpDir, "src", "wrappers")
	if err := os.Mkdir(wrappersDir, 0700); err != nil {
		t.Fatalf("creating wrappers dir failed: %v", err)
	}
	...
	if err := os.CopyFS(wrappersDir, os.DirFS("../../testdata/src/a/wrappers")); err != nil {
		t.Fatalf("populating temp dir with wrappers package failed: %v", err)
	}
...
func main() {
	...
	w.MyEventually(1)
	w.MyConsistently(1)
	w.MyConsistently(1).WithTimeout(0).WithPolling(0)
	w.NewAssertion(1)
	w.NewAsyncAssertion(1)
...
`,
			...
			expectInfos: []*GomegaBasicInfo{
				...
				{MethodName: "MyConsistently", RootCallType: AsyncAssertionCall},
				{MethodName: "MyConsistently", RootCallType: AsyncAssertionCall},
				{MethodName: "NewAssertion", RootCallType: SyncAssertionCall},
				{MethodName: "NewAsyncAssertion", RootCallType: AsyncAssertionCall},
