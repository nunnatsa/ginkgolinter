	...
	if !bool(config.SuppressLen) && isActualIsLenFunc(actualArg) {
		...
		if nilable, compOp := getNilableFromComparison(actualArg); nilable != nil {
			...
			if !bool(config.SuppressLen) && isActualIsLenFunc(first) {
				return handleLenComparison(pass, assertionExp, first, second, op, handler, oldExpr)
			}
			return bool(config.SuppressCompare) || checkComparison(assertionExp, pass, handler, first, second, op, config, oldExpr)
			...
			return handleAssertionOnly(pass, config, assertionExp, handler, oldExpr, true)
		}
	}
}
...
func checkAsyncAssertion(pass *analysis.Pass, config types.Config, expr *ast.CallExpr, actualExpr *ast.CallExpr, handler gomegahandler.Handler, oldExpr string) bool {
		...
		if len(actualExpr.Args) > funcIndex {
			if fun, funcCall := actualExpr.Args[funcIndex].(*ast.CallExpr); funcCall {
				...
				default:
					...
					handleAssertionOnly(pass, config, expr, handler, oldExpr, false)
	...
	handleAssertionOnly(pass, config, expr, handler, oldExpr, true)
	return true
}
...
func checkComparison(exp *ast.CallExpr, pass *analysis.Pass, handler gomegahandler.Handler, first ast.Expr, second ast.Expr, op token.Token, config types.Config, oldExp string) bool {
	matcher, shouldContinue := startCheckComparison(exp, handler)
	if !shouldContinue {
		return true
	}
...
func handleLenComparison(pass *analysis.Pass, exp *ast.CallExpr, first ast.Expr, second ast.Expr, op token.Token, handler gomegahandler.Handler, oldExpr string) bool {
	matcher, shouldContinue := startCheckComparison(exp, handler)
	if !shouldContinue {
		return true
	}
...
//	Equal(false) => BeFalse()
//	HaveLen(0) => BeEmpty()
func handleAssertionOnly(pass *analysis.Pass, config types.Config, assertionExp *ast.CallExpr, handler gomegahandler.Handler, oldExpr string, shouldReport bool) bool {
	if len(assertionExp.Args) == 0 {
		return true
	}
	...
	case not:
		reverseAssertionFuncLogic(assertionExp)
		assertionExp.Args[0] = assertionExp.Args[0].(*ast.CallExpr).Args[0]
		return handleAssertionOnly(pass, config, assertionExp, handler, oldExpr, shouldReport)
	default:
		return true
	}
}
