...
var _ = Describe("check async intervals", func() {
	...
	It("mix multiple timeout &polling", func() {
		Eventually(func() bool { return true }).WithTimeout(time.Second * 10).WithPolling(500 * time.Millisecond).WithTimeout(time.Second * 10).WithPolling(500 * time.Millisecond).Should(BeTrue()) // want `multiple issues: polling defined more than once; timeout defined more than once`
		Eventually(func() bool { return true }).WithTimeout(time.Second * 10).WithPolling(500 * time.Millisecond).Within(time.Second * 10).ProbeEvery(500 * time.Millisecond).Should(BeTrue())       // want `multiple issues: polling defined more than once; timeout defined more than once`
		Eventually(func() bool { return true }).Within(time.Second * 10).ProbeEvery(500 * time.Millisecond).Within(time.Second * 10).ProbeEvery(500 * time.Millisecond).Should(BeTrue())             // want `multiple issues: polling defined more than once; timeout defined more than once`
		Eventually(func() bool { return true }, time.Second*10, time.Millisecond*500).WithTimeout(time.Second * 60 * 60 * 24 * 365).WithPolling(time.Second * (10 + factor)).Should(BeTrue())        //want `multiple issues: polling defined more than once; timeout defined more than once`
		Eventually(func() bool { return true }, time.Second*10, time.Millisecond*500).Within(time.Second * 60 * 60 * 24 * 365).ProbeEvery(time.Second * (10 + factor)).Should(BeTrue())              //want `multiple issues: polling defined more than once; timeout defined more than once`
		Eventually(func() bool { return true }, time.Second*10, time.Millisecond*500).WithTimeout(time.Second * 60 * 60 * 24 * 365).ProbeEvery(time.Second * (10 + factor)).Should(BeTrue())         //want `multiple issues: polling defined more than once; timeout defined more than once`
		Eventually(func() bool { return true }, time.Second*10, time.Millisecond*500).Within(time.Second * 60 * 60 * 24 * 365).WithPolling(time.Second * (10 + factor)).Should(BeTrue())             //want `multiple issues: polling defined more than once; timeout defined more than once`
	})
})
