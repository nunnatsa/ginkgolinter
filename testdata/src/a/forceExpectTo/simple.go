...
var _ = Describe("simple case", func() {
	It("just replace assertion method", func() {
		...
		ExpectWithOffset(1, "a").Should(Not(BeEmpty())) // want `ginkgo-linter: must not use ExpectWithOffset with Should. Consider using .ExpectWithOffset\(1, "a"\).ToNot\(BeEmpty\(\)\). instead`
	...
	It("mix with len assertion", func() {
		...
		ExpectWithOffset(1, len("a")).ShouldNot(Equal(0)) // want `ginkgo-linter: multiple issues: must not use ExpectWithOffset with ShouldNot; wrong length assertion. Consider using .ExpectWithOffset\(1, "a"\)\.ToNot\(BeEmpty\(\)\). instead`
	})
